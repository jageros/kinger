...
func doInitAllGoods(gdata gamedata.IGameData, isReload bool) {
	htype2Goods = map[pb.HuodongTypeEnum]map[int]iHuodongGoods{}
	huodongGameData := gdata.(*gamedata.HuodongGameData)
	if huodongGameData.ID2Huodong == nil {
		return
	}

	for id, hdata := range huodongGameData.ID2Huodong {
		htype := pb.HuodongTypeEnum(id)
		goodsGameData := gamedata.GetGameData(hdata.Reward)
...
func initAllGoods() {
	...
	huodongGameData.AddReloadCallback(func(data gamedata.IGameData) {
		doInitAllGoods(data, true)
	})
