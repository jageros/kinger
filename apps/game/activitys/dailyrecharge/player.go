...
func newComponent(player types.IPlayer) *activityCom {
	...
	return &activityCom{
		ipc:    pcm,
		player: player,
		attr:   pcm.InitAttr(aTypes.DailyRechargeActivity),
	}
}
...
func (c *activityCom) getTodayRechargeAmount(aid int) int {
	...
	return attr.GetInt(aTypes.TodayRechargeAmount)
}

func (c *activityCom) setTodayRechargeAmount(activityID, money int) {
	...
	oldNum := attr.GetInt(aTypes.TodayRechargeAmount)
	newNum := oldNum + money
	attr.SetInt(aTypes.TodayRechargeAmount, newNum)
...
func (c *activityCom) setReceive(activityID, rewardID int) error {
	...
	rwAttr := attr.GetMapAttr(aTypes.DailyRechargeReceiveBool)
	if rwAttr == nil {
		rwAttr = attribute.NewMapAttr()
		attr.SetMapAttr(aTypes.DailyRechargeReceiveBool, rwAttr)
	}
...
func (c *activityCom) conformRewardCondition(activityID, rewardID int) bool {
	...
	rechargeAmount := c.getTodayRechargeAmount(activityID)
...
func (c *activityCom) OnCrossDays(dayno int) {
	...
	aTypes.IMod.ForEachActivityDataByType(consts.ActivityOfDailyRecharge, func(data aTypes.IActivity) {
