...
func newComponent(player types.IPlayer) *activityCom {
	...
	return &activityCom{
		ipc:    pcm,
		player: player,
		attr:   pcm.InitAttr(aTypes.FightActivity),
	}
}
...
func (c *activityCom) setActivityVersion(activityID int) {
	...
	if activity == nil {
		...
		glog.Errorf("Fight activity setActivityVersion GetActivityByID err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), aid)
		return
	}
	attr.SetInt(aTypes.Version, activity.GetActivityVersion())
}

func (c *activityCom) getTotalFightNum(activityID int) int {
	aid := strconv.Itoa(activityID)
	attr := c.attr.GetMapAttr(aid)
	if attr == nil {
		return 0
	}
	return attr.GetInt(aTypes.TotalFightAmount)
}

func (c *activityCom) setTotalFightNum() {
	...
	for _, aid := range idList {
		if c.ipc.Conform(aid) {
			...
			oldm := attr.GetInt(aTypes.TotalFightAmount)
			newm := oldm + 1
			attr.SetInt(aTypes.TotalFightAmount, newm)
			c.ipc.PushFinshNum(aid, 0, newm)
			c.updateReceiveStatus(aid, oldm, newm)
		}
	}
}
...
func (c *activityCom) setReceive(activityID, rewardID int) error {
	...
	if activity == nil {
		err := gamedata.GameError(aTypes.GetActivityError)
		glog.Errorf("Fight activity setReceive GetActivityByID err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), activityID, rewardID)
		return err
	}
	aid := strconv.Itoa(activityID)
	...
	rwAttr := attr.GetMapAttr(aTypes.FightRewardBool)
	if rwAttr == nil {
		rwAttr = attribute.NewMapAttr()
		attr.SetMapAttr(aTypes.FightRewardBool, rwAttr)
	}
...
func (c *activityCom) hasReceive(activityID, rewardID int) (bool, error) {
	...
	if activity == nil {
		err := gamedata.GameError(aTypes.GetActivityError)
		glog.Errorf("Fight activity hasReceive GetActivityByID err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false, err
	}
	aid := strconv.Itoa(activityID)
	rid := strconv.Itoa(rewardID)
...
func (c *activityCom) conformRewardCondition(activityID, rewardID int) bool {
	...
	if activity == nil {
		err := gamedata.GameError(aTypes.GetActivityError)
		glog.Errorf("Fight activity conformRewardCondition GetActivityByID err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false
	}

	received, err := c.hasReceive(activityID, rewardID)
	if err != nil {
		glog.Errorf("Fight activity conformRewardCondition hasReceive return err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false
	}
	...
	if err != nil {
		glog.Errorf("Fight activity conformRewardCondition getRewardFinshCondition return err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false
	}
	hasNum := c.getTotalFightNum(activityID)
...
func (c *activityCom) getRewardReceiveStatus(aid, rid int) (rst pb.ActivityReceiveStatus) {
	...
	if err != nil {
		glog.Errorf("Fight activity getRewardReceiveStatus hasReceive return err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), aid, rid)
		return
	}
	if hasReceive {
...
func (c *activityCom) giveReward(activityID, rewardID int, rd *pb.Reward) error {
	...
	for stuff, num := range rw {
		...
		if err != nil {
			glog.Errorf("Fight activity giveReward GiveReward return err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), activityID, rewardID)
			return err
		}
	}
...
func (c *activityCom) checkVersion(aid int) bool {
	...
	if act == nil {
		glog.Errorf("fight activity get activity by id error, activityID=%d, uid=%d", aid, c.player.GetUid())
		return false
	}
...
func (c *activityCom) updateReceiveStatus(aid, oldNum, newNum int) {
	...
	for _, rid := range rids {
		...
		if err != nil {
			glog.Error("Fight activity updateReceiveStatus getRewardFinshCondition err=%s, uid=%d, activityID=%d, RewardID=%d", err, c.player.GetUid(), aid, rid)
		}
...
func (c *activityCom) updateActivityCloseStatus() {
	for aid, rw := range mod.id2Reward {
		flag := 0
		if len(rw.ActivityFightRewardMap) <= 0 {
			continue
		}
		for rid, _ := range rw.ActivityFightRewardMap {
			b, err := c.hasReceive(aid, rid)
			if err != nil {
				glog.Errorf("Fight activity updateActivityCloseStatus hasReceive return err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), aid, rid)
				break
			}
			if !b {
				flag = 1
