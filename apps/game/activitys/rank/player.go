...
func newComponent(player types.IPlayer) *activityCom {
	...
	return &activityCom{
		ipc:    pcm,
		player: player,
		attr:   pcm.InitAttr(aTypes.RankActivity),
	}
}
...
func (c *activityCom) setActivityVersion(activityID int) {
	...
	if activity == nil {
		...
		glog.Errorf("Rank activity setActivityVersion GetActivityByID err=%s, uid=%d, activityID=%d", err, c.player.GetUid(), aid)
		return
	}
...
func (c *activityCom) setReceive(activityID, rewardID int) error {
	...
	if activity == nil {
		err := gamedata.GameError(aTypes.GetActivityError)
		glog.Errorf("Rank activity setReceive GetActivityByID err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), activityID, rewardID)
		return err
	}
	aid := strconv.Itoa(activityID)
	...
	rwAttr := attr.GetMapAttr(aTypes.RankRewardBool)
	if rwAttr == nil {
		rwAttr = attribute.NewMapAttr()
		attr.SetMapAttr(aTypes.RankRewardBool, rwAttr)
	}
...
func (c *activityCom) hasReceive(activityID, rewardID int) (bool, error) {
	...
	if activity == nil {
		err := gamedata.GameError(aTypes.GetActivityError)
		glog.Errorf("Rank activity hasReceive GetActivityByID err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false, err
	}
	aid := strconv.Itoa(activityID)
	rid := strconv.Itoa(rewardID)
...
func (c *activityCom) conformRewardCondition(activityID, rewardID int) bool {
	...
	if activity == nil {
		err := gamedata.GameError(aTypes.GetActivityError)
		glog.Errorf("Rank activity conformRewardCondition GetActivityByID err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false
	}

	received, err := c.hasReceive(activityID, rewardID)
	if err != nil {
		glog.Errorf("Rank activity conformRewardCondition hasReceive err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false
	}
	...
	if err != nil {
		glog.Errorf("Rank activity conformRewardCondition getRewardFinshCondition err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), activityID, rewardID)
		return false
	}
	hasNum := c.player.GetMaxPvpLevel()
...
func (c *activityCom) getRewardReceiveStatus(aid, rid int) (rst pb.ActivityReceiveStatus) {
	...
	if err != nil {
		glog.Errorf("Rank activity getRewardReceiveStatus hasReceive err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), aid, rid)
		return
	}
	if hasReceive {
...
func (c *activityCom) checkVersion(aid int) bool {
	...
	if act == nil {
		glog.Errorf("Rank activity get activity by id error, activityID=%d, uid=%d", aid, c.player.GetUid())
		return false
	}
...
func (c *activityCom) updateActivityCloseStatus() {
	for aid, rw := range mod.id2Reward {
		flag := 0
		if len(rw.ActivityRankRewardMap) <= 0 {
			continue
		}
		for rid, _ := range rw.ActivityRankRewardMap {
			b, err := c.hasReceive(aid, rid)
			if err != nil {
				glog.Errorf("Rank activity updateActivityCloseStatus hasReceive return err=%s, uid=%d, activityID=%d, rewardID=%d", err, c.player.GetUid(), aid, rid)
				break
			}
			if !b {
				flag = 1
