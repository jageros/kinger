...
func (monitor *_Monitor) Dump() {
	...
	for _, _t := range copyOpInfos {
		...
		if falconAgentUrl != "" {
			...
			glog.Infof("monitor Dump ==== %-30sx%-10d AVG %-10s MAX %-10s", opname, opinfo.count,
	...
	if len(falconInfos) > 0 {
		...
		if err != nil {
			glog.Errorf("Post falconAgent %s fail %s", falconAgentUrl, err)
			return
		}
	}
}
...
func (op *Operation) Finish(warnThreshold time.Duration) {
	...
	if warnThreshold > 0 && takeTime >= warnThreshold {
		glog.Warnf("opmon: operation %s takes %s > %s", op.name, takeTime, warnThreshold)
	}
