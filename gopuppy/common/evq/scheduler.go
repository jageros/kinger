...
func (es *eventScheduler) doScheduler() {
	for {
		ev := es.queue.pop()
		if ev == nil {
			glog.Errorf("doScheduler es.queue.pop %s", ev)
			continue
		}
		evId := ev.GetEventId()
		if evId == consts.ClOSE_EVQ_EVT {
			...
			if evId == consts.CALLATER_EVT {
				if e, ok := ev.(*callLaterEvent); ok {
					...
					glog.Errorf("evq doScheduler CALLATER_EVT not a callLaterEvent")
				}
			} else {
				es.onEvent(ev)
			}
			...
			if co, ok := ev.(*Coroutine); ok {
				...
				glog.Errorf("evq doScheduler COROUTINE_EVENT not a Coroutine")
			}
		}
...
func (es *eventScheduler) waitClear() {
	...
	case <-time.NewTimer(10 * time.Second).C:
		glog.Errorf("eventScheduler waitClear timeout queue=%s", es)
		return
	}
}
