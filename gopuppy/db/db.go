...
import (
	...
	"kinger/gopuppy/db/backend/mongo"
...
func GetOrNewDbClient(cfg IDbConfig) IDbClient {
	...
	if err != nil {
		glog.Fatalf("db engine %s is not ready: %s", cfg, err)
	}
...
func (c *dbClient) checkOperationQueueLen() {
	...
	if qlen > 100 && qlen%100 == 0 && c.recentWarnedQueueLen != qlen {
		glog.Warnf("db %s operation queue length = %d", c.cfg, qlen)
	...
	c.shutdownOnce.Do(func() {
		...
		for c.operationQueue.Len() > 0 {
			if waitTime > 10*time.Second {
				glog.Errorf("db %s Shutdown timeout, left op %d", c.cfg, c.operationQueue.Len())
				break
			}
...
func (c *dbClient) dbRoutine() {
	defer func() {
		err := recover()
		if err != nil {
			glog.TraceError("db %s routine paniced: %s", c.cfg, err)
		} else {
			c.dbEngine.Close()
			c.dbEngine = nil
	...
	for {
		...
		if err != nil {
			glog.Errorf("db %s engine is not ready: %s", c.cfg, err)
			time.Sleep(time.Second)
			continue
		}

		if c.dbEngine == nil {
			glog.Fatalf("db %s engine is nil", c.cfg)
		}
		...
		if !ok {
			glog.Errorf("db: unknown operation: %v", req)
			continue
		}
		...
		if err != nil {
			glog.Errorf("db: %s %s failed: %s", c.cfg, req2.name(), err)
